...
function Show-ImageFormats {
    ...
    foreach ($key in ($imageFormatosSalida.Keys | Sort-Object { [int]$_ })) {
        Write-Log "    [$key] $($imageFormatosSalida[$key].name)" "INFO" "White"
    }
    Write-Log "" "" "White"
}
...
function Convert-Images {
    ...
    if ($archivos.Count -eq 0) {
        Write-Log "No images to convert in INPUT/images/" "WARN" "Yellow"
        return 0
    }

    Write-Log "" "" "Cyan"
    Write-Log "Converting $($archivos.Count) image(s) to $($formato.name)..." "INFO" "Cyan"
    Write-Log "" "" "Cyan"
